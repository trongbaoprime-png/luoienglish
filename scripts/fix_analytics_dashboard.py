"""Fix Customer Analytics Dashboard: Separate customer_data.js and implement offline native Canvas charts."""
...
def fix_dashboard():
    ...
    print(f"[FIXER] Loaded {len(customers):,} customer records.")

    # Compact array format: [date, name, phone, source, branch, service, staff]
    compact_list = []
    for c in customers:
        compact_list.append([
            c.get("date", ""),
    ...
    # 1. Save customer_data.js
    data_js_content = f"window.CUSTOMERS = {json.dumps(compact_list, ensure_ascii=False)};\nwindow.ROI_DATA = {json.dumps(roi_data, ensure_ascii=False)};"
    Path("customer_data.js").write_text(data_js_content, encoding="utf-8")
    print(f"[FIXER] Generated customer_data.js ({len(data_js_content):,} bytes).")

    # 2. Save robust customer_analytics_dashboard.html
...
<div class="container">
  <header>
    <div>
      <div class="brand-title">
        ...
        <span class="badge-tag" id="headerCountBadge">48,617 Khách Hàng</span>
      </div>
  ...
  <div class="table-wrap">
    ...
    <div class="pagination">
      <div id="pageInfo">Đang tải dữ liệu...</div>
...
<!-- Load External Fast Customer Data JS -->
...
function init() {
  ...
  console.log("Loaded customers count:", data.length);
  filteredData = [...data];

  // Populate Branch Filter Select
...
function renderTable() {
  ...
  pageItems.forEach((c, idx) => {
    // c = [date, name, phone, source, branch, service, staff]
    tbody.innerHTML += `
      <tr>
...
function onFilterChange() {
  ...
  filteredData = data.filter(c => {
    // c = [date(0), name(1), phone(2), source(3), branch(4), service(5), staff(6)]
    ...
    if (s !== 'all') {
      const svc = String(c[5]).toUpperCase();
...
function renderNativeTrendChart() {
  ...
  values.forEach((val, i) => {
    ...
    // Rounded bar top
    ctx.beginPath();
    ctx.roundRect(x, y, barW - 6, barH, [4, 4, 0, 0]);
    ctx.fill();

    // Value label
    ctx.fillStyle = '#0f172a'; ctx.font = '10px Plus Jakarta Sans'; ctx.textAlign = 'center';
    ctx.fillText((val / 1000).toFixed(1) + 'k', x + (barW - 6) / 2, y - 5);

    // X Label
    ctx.fillStyle = '#64748b'; ctx.font = '9px Plus Jakarta Sans';
...
function renderNativePieChart() {
  ...
  data.forEach(c => {
    const s = String(c[5]).toUpperCase();
  ...
  const slices = [
    ...
    { label: 'Tổng Quát / Khác', count: khac, color: '#94a3b8' }
  ];
...
"""
    Path("customer_analytics_dashboard.html").write_text(html_content, encoding="utf-8")
    print(f"[FIXER] Successfully updated customer_analytics_dashboard.html!")
