...
export class FirestoreReviewAttemptTransactionRepository
  implements IReviewAttemptTransactionRepository
{
  private reviewSessionsCollection = "reviewSessions";
  private knowledgeMasteryCollection = "knowledgeMastery";
  ...
  public async executeAttemptTransaction(
    ...
    const sessionDocRef = doc(db, this.reviewSessionsCollection, sessionId);

    return await runTransaction(db, async (t) => {
      // 1. Transactional Read: ReviewSession
      const sessionSnap = await t.get(sessionDocRef);
      if (!sessionSnap.exists()) {
        throw new ServerAuthError(`Không tìm thấy phiên ôn tập '${sessionId}'.`, 404);
      }

      const session = sessionSnap.data() as ReviewSession;
      ...
      if (existingEvidence) {
        // Zero writes in transaction on idempotent hit
        return {
          evaluation: {
      ...
      const masteryDocId = `${session.childId}_${currentItem.knowledgeId}`;
      const masteryDocRef = doc(db, this.knowledgeMasteryCollection, masteryDocId);
      const masterySnap = await t.get(masteryDocRef);

      const existingMastery: KnowledgeMastery = masterySnap.exists()
        ? (masterySnap.data() as KnowledgeMastery)
        : {
      ...
      // 9. True Atomic Commit: Both writes committed in single Firestore runTransaction
      t.set(sessionDocRef, updatedSession);
      t.set(masteryDocRef, updatedMastery, { merge: true });
