import {
  IReviewAttemptTransactionRepository,
  ExecuteReviewAttemptTransactionParams,
  ReviewAttemptTransactionOutput,
} from "../interfaces/IReviewAttemptTransactionRepository";
import { InMemoryReviewSessionRepository } from "./InMemoryReviewSessionRepository";
...
export class InMemoryReviewAttemptTransactionRepository
  implements IReviewAttemptTransactionRepository
{
  private simulatePreCommitFailure = false;

  constructor(
    private sessionRepo: InMemoryReviewSessionRepository,
    private memoryRepo: InMemoryMemoryRepository
  ) {}

  /**
   * Test Hook: Enables simulation of an unexpected process/datastore crash
   * immediately before writes are committed.
   */
  public setSimulatePreCommitFailure(shouldFail: boolean): void {
    this.simulatePreCommitFailure = shouldFail;
  }

  public async executeAttemptTransaction(
    ...
    // 2. In-Transaction Concurrency Version Gate
    ...
    // 3. In-Transaction Attempt Idempotency Gate
    ...
    // 4. In-Transaction Activity Sequence Gate
    ...
    // 5. Sanitize Low-Trust Telemetry
    ...
    // 7. Transactional Read: KnowledgeMastery
    const now = new Date().toISOString();
    const existingMastery =
      (await this.memoryRepo.getMastery(session.childId, currentItem.knowledgeId)) || {
    ...
    // 9. Failure Injection Check (Simulate crash before atomic commit)
    if (this.simulatePreCommitFailure) {
      throw new Error("SIMULATED_DATASTORE_TRANSACTION_FAILURE: Crash before commit");
    }

    // 10. Atomic Commit of both documents
    await this.sessionRepo.saveSession(updatedSession);
    await this.memoryRepo.saveMastery(updatedMastery);
